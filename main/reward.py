...
def reward_simple_sync2(world, agent):
    ...
    flash_tolerance = 0.1
    flash_tolerance_extra = 1.75*flash_tolerance
    flash_timeout = 0.5
    ...
    # If you are flashing.
    if flash:
        flash_accepted = (steps_since_flash >= 2)
        if timer <= flash_tolerance/2 or (1-timer) <= flash_tolerance/2:
            reward += 100 if flash_accepted else 0
        elif timer <= flash_tolerance_extra/2 or (1-timer) <= flash_tolerance_extra/2:
            reward += 10 if flash_accepted else 0
        else:
            reward -= 100
    else:
        reward += 0
...
# Synchronize with other agents.
def reward_simple_sync3(world, agent):
    # Get flash information.
    flash = world.get(agent, 'action', standardized=False)
    steps_since_flash = world.get(agent, 'steps_since_flash')
    steps_since_flash_absolute = world.get(agent, 'steps_since_flash', standardized=False)
    ...
    if flash:
        if timer <= flash_tolerance:
            reward += 100
        elif timer <= 1.5*flash_tolerance:
            reward += 50
        else:
            reward -= 20
